...
import com.project.drama.config.KafkaProperties;
...
class DramaKafkaResourceIT {
    ...
    void setup() {
        ...
        DramaKafkaResource kafkaResource = new DramaKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/drama-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/drama-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
